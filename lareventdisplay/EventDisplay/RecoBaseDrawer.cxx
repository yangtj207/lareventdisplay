...
namespace evd
{
                                ...
                                unsigned int plane)
    {
        ...
        if (rawOpt->fDrawRawDataOrCalibWires < 1)
            return;
            ...
            for (size_t i = 0; i < wires.size(); ++i)
            {
                ...
                for (auto const &wid : wireids)
                {
                    ...
                    while (itr != wirSig.end())
                    {
                        ...
                        if (TMath::Abs(adc) < rawOpt->fMinSignal)
                            continue;
                        if (tdc > rawOpt->fTicks)
                            continue;
        ...
        for (size_t imod = 0; imod < recoOpt->fHitLabels.size(); ++imod)
        {
            ...
            for (auto itr : hits)
            {
                ...
                if (itr->WireID().TPC != rawOpt->fTPC ||
                    itr->WireID().Cryostat != rawOpt->fCryostat)
                    continue;
                              ...
                              int lineWidth)
    {
        ...
        for (const auto &hit : hits)
        {
            ...
            {
                if (wireID.TPC != rawOpt->fTPC || wireID.Cryostat != rawOpt->fCryostat)
                    continue;
                                   ...
                                   unsigned int plane)
    {
        ...
        for (size_t imod = 0; imod < recoOpt->fOpFlashLabels.size(); ++imod)
        {
            ...
            {
                if (opflashes[iof]->TotalPE() < recoOpt->fFlashMinPE)
                    continue;
                if (opflashes[iof]->Time() < recoOpt->fFlashTMin)
                    continue;
                if (opflashes[iof]->Time() > recoOpt->fFlashTMax)
                    continue;
                ...
                for (size_t i = 0; i < points.size(); ++i)
                {
                    ...
                    if (wireID.Wire < wire0)
                        wire0 = wireID.Wire;
                    if (wireID.Wire > wire1)
                        wire1 = wireID.Wire;
                }
                if (rawOpt->fAxisOrientation > 0)
                {
        ...
        for (size_t imod = 0; imod < recoOpt->fSliceLabels.size(); ++imod)
        {
            ...
            for (size_t isl = 0; isl < slices.size(); ++isl)
            {
                ...
                if (recoOpt->fDrawSlices < 3)
                {
                    ...
                    if (this->Hit2D(hits_on_plane, color, view, false, false) < 1)
                        continue;
                    if (recoOpt->fDrawSlices == 2)
                    {
                        geo::Point_t slicePos(slices[isl]->Center().X(),slices[isl]->Center().Y(),slices[isl]->Center().Z());
                ...
                else
                {
                    ...
                    geo::Point_t slicePos0(slices[isl]->End0Pos().X(),slices[isl]->End0Pos().Y(),slices[isl]->End0Pos().Z());
                    ...
                    geo::Point_t slicePos1(slices[isl]->End1Pos().X(),slices[isl]->End1Pos().Y(),slices[isl]->End1Pos().Z());
        ...
        for (size_t imod = 0; imod < recoOpt->fClusterLabels.size(); ++imod)
        {
                ...
                if (spHitAssnVec.isValid())
                {
                    ...
                    for (const auto &spacePointPtr : spacePointVec)
                    {
                        if (spacePointPtr->Chisq() < -99.)
                        {
                            ...
                            {
                                if (hitPtr.get()->WireID().Plane != plane)
                                    continue;
            ...
            for (size_t ic = 0; ic < clust.size(); ++ic)
            {
                ...
                if (drawAsMarkers)
                {
                    ...
                    if (this->Hit2D(hits, color, view, false, drawConnectingLines) < 1)
                        continue;
                    ...
                    if (recoOpt->fDrawClusters > 3)
                    {
                        ...
                        if (pfpIndex != INT_MAX)
                            s = s + " P" + std::to_string(pfpIndex + 1);
                        char const *txt = s.c_str();
                                              ...
                                              evdb::View2D *view)
    {
        ...
        if (recoOpt->fDraw2DSlopeEndPoints < 1)
            return;
                                            ...
                                            unsigned int plane)
    {
        ...
        for (size_t j = 0; j < hits.size(); ++j)
        {
            ...
            if (t < wlo[hits[j]->WireID().Wire])
                wlo[hits[j]->WireID().Wire] = t;
            if (t > whi[hits[j]->WireID().Wire])
                whi[hits[j]->WireID().Wire] = t;
        }
            ...
            for (size_t imod = 0; imod < recoOpt->fTrackLabels.size(); ++imod)
            {
                ...
                for (size_t t = 0; t < track.vals().size(); ++t)
                {
                    ...
                    if (recoOpt->fDrawTracks > 1)
                    {
                        ...
                        geo::Point_t trackPos(track.vals().at(t)->End().X(),track.vals().at(t)->End().Y(),track.vals().at(t)->End().Z());
            ...
            for (size_t imod = 0; imod < recoOpt->fShowerLabels.size(); ++imod)
            {
                ...
                for (size_t s = 0; s < shower.vals().size(); ++s)
                {
                    ...
                    if (recoOpt->fDrawShowers > 1)
                    {
                        ...
                        if (!shower.vals().at(s)->has_length())
                            continue;
                        if (!shower.vals().at(s)->has_open_angle())
                            continue;
                                                ...
                                                unsigned int plane)
    {
        ...
        if (!recoOpt->fDrawTrackVertexAssns)
            return;
        ...
        for (size_t imod = 0; imod < recoOpt->fTrkVtxTrackLabels.size(); ++imod)
        {
            ...
            if (vertexTrackAssnsHandle->size() < 1)
                continue;
                                  ...
                                  unsigned int plane)
    {
        ...
        for (size_t imod = 0; imod < recoOpt->fVertexLabels.size(); ++imod)
        {
            ...
            for (size_t v = 0; v < vertex.size(); ++v)
            {
                ...
                double xyz[3];
                vertex[v]->XYZ(xyz);
                if (xyz[0] < minxyz[0] || xyz[0] > maxxyz[0])
                    continue;
                if (xyz[1] < minxyz[1] || xyz[1] > maxxyz[1])
                    continue;
                if (xyz[2] < minxyz[2] || xyz[2] > maxxyz[2])
                    continue;
                
                geo::Point_t localPos(xyz[0],xyz[1],xyz[2]);

                // BB: draw polymarker at the vertex position in this plane
                double wire = geo->WireCoordinate(localPos, geo::PlaneID(rawOpt->fCryostat, rawOpt->fTPC, plane));
...
*/
                                 ...
                                 evdb::View3D *view)
    {
        ...
        if (rawOpt->fDrawRawDataOrCalibWires < 1)
            return;
        if (recoOpt->fDrawSlices < 1)
            return;
        if (recoOpt->fDrawSliceSpacePoints < 1)
            return;
        for (size_t imod = 0; imod < recoOpt->fSliceLabels.size(); ++imod)
        {
            art::InputTag const which = recoOpt->fSliceLabels[imod];
                                  ...
                                  unsigned int plane)
    {
        ...
        {
            if (temp[t]->WireID().Cryostat == cryostat && temp[t]->WireID().TPC == tpc && temp[t]->WireID().Plane == plane)
                break;
